...
public class SH_Calculation : MonoBehaviour
{
    ...
    private void Calculation(int index)
    {
        ...
        Graphics.Blit(null, _temp0, _material0);   // 球面調和係数を計算
        Graphics.Blit(_temp0, _temp1, _material1); // mipmapを利用して球面調和係数の平均を計算

        // 球面調和係数の平均を計算した RenderTexture のデータを取得する
        // AsyncGPUReadback は Editor 上で動かないので使わない
