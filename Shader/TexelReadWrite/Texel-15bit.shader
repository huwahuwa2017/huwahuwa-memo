Shader "TexelReadWrite/Texel-15bit"
{
	SubShader
	{
		Tags
		...
		Pass
		{
			...
			void GeometryShaderStage(triangle V2G input[3], inout TriangleStream<G2F> stream)
			{
				...
				// 同じパラメータを持った頂点が合成される説と
				// 三角形を構成する最初の頂点しか取得しない説がある
				// おそらく後者が正しい
				...
				data0.xyz = HTRW_R15BIT_TO_FP16(temp0);
				data1.xyz = HTRW_R15BIT_TO_FP16(temp0 >> 15);
				temp0 &= 0xC0000000;
				data0.w = HTRW_R15BIT_TO_FP16((temp0.x >> 30) | (temp0.y >> 28) | (temp0.z >> 26));
				data1.w = 0.0;
		...
		Pass
		{
			...
			V2F VertexShaderStage(I2V input)
			{
				...
				uint4 data2 = HTRW_FP16_TO_R15BIT(data0);
				uint4 data3 = HTRW_FP16_TO_R15BIT(data1);
