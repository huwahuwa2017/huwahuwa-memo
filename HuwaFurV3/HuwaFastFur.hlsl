...
V2G VertexStage(I2V input, uint id : SV_VertexID)
{
...
V2G HullStage(InputPatch<V2G, 3> input, uint id : SV_OutputControlPointID)
{
    return input[id];
}
...
[domain("tri")]
V2G DomainStage(TessellationFactor tf, const OutputPatch<V2G, 3> input, float3 bary : SV_DomainLocation)
{
    ...
    output.id = ValueToRandom(uint4(input[0].id, input[1].id, input[2].id, ValueToRandom(bary)));
    output.furCount = tf.furCount;
    return output;
}

[maxvertexcount(_MaxVertexCount)]
void GeometryStage(triangle V2G input[3], inout TriangleStream<G2F> stream)
{
    ...
    uint random = ValueToRandom(uint3(input[0].id, input[1].id, input[2].id));
    ...
    for (int count = 0; count < targetfurcount; ++count)
    {
        ...
        output.transparency = _FurSplitTemp;
        stream.Append(output);
...
half4 FragmentStage(G2F input) : SV_Target
{
    float temp0 = input.transparency + 0.33;
    float temp1 = frac(temp0);
    float temp2 = lerp(temp1, 1.0 - temp1, floor(temp0 % 2.0));
    temp2 = temp2 * 3.0 - 1.0;
    clip(temp2);
    temp2 = saturate(temp2);
    
    half3 col = tex2D(_MainTex, input.uv).rgb * input.lightColor;
    return half4(col, temp2 * temp2);
}
